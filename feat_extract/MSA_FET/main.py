...
class FeatureExtractionTool(object):
    ...
    def __init__(
        ...
        tmp_dir=osp.join('feat_extract/MMSA-FET/tmp'),
        log_dir=osp.join('feat_extract/MMSA-FET/log'),
        verbose=1
    ):
        if type(config) == dict:
            self.config = config
            ...
            raise ValueError("Inval config type.")
        ...
        if self.verbose == 1:
            ...
            raise ValueError(f"Inval verbose level '{self.verbose}'.")
    ...
    def __text_extract_single(self, in_file):
        ...
        text_bert_result = self.text_extractor.tokenize(text)
        return text_result, text_bert_result
    ...
    def run_single(self, in_file, out_file=None, text_file=None, return_type='np'):
        ...
        try:
            ...
            if 'audio' in self.config:
                ...
                final_result['audio_lengths'] = audio_result.shape[0]
                print('audio feat shape:', audio_result.shape)
            if 'video' in self.config:
                video_result = self.__video_extract_single(in_file)
                final_result['vision'] = video_result
                final_result['vision_lengths'] = video_result.shape[0]
                print('vision feat shape:', video_result.shape)
            if 'text' in self.config:
                if text_file is None:
                    raise ValueError("Text file is not specified.")
                text_result, text_bert_result = self.__text_extract_single(text_file)
                #text_bert_result = text_bert_result.transpose(0, 2, 1)
                final_result['text'] = text_result
                final_result['text_bert'] = text_bert_result
                print('text feat shape:', text_result.shape)
                print('text bert feat shape:', text_bert_result.shape)
                if text_result.shape[0] != text_bert_result.shape[0]:
                    print('##################################')
                    print('text shape:',text_result.shape)
                    print('text bert shape:', text_bert_result.shape)
                final_result['text_lengths'] = text_result.shape[0]
            ...
            #         final_result['text_bert'] = torch.from_numpy(text_bert_result)
            # else:
            #     raise ValueError(f"Inval return type '{return_type}'.")
            # save result
            if out_file:
    ...
    def run_dataset(self, dataset_name=None, dataset_root_dir=None, dataset_dir=None, out_file=None, return_type='np', num_workers=4,
        ...
        try:
            ...
            data.pop('mode')
            final_data = {k: {} for k in ['train', 'val', 'test']}
            for mode in ['train', 'val', 'test']:
