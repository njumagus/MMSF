...
from .data_loader_lvu import LVUDataLoader
...
SUPPORTED_DATASETS = ['MOSI', 'MOSI_LVU', 'LVU', 'MOSEI', 'SIMS']
...
def MMSA_run_test(model_name, dataset_name, config=None, config_file="", seeds=[], is_tune=False,
    tune_times=50, feature_T="", feature_A="", feature_V="",
    model_save_dir="", res_save_dir="", log_dir="",
    gpu_ids=[0], num_workers=4, verbose_level=1,train_mode='regression', get_feats=True):
    ...
    if 'lvu' == args['dataset_name'] and get_feats:
        args['model_save_path'] = Path(model_save_dir) / f"{args['model_name']}-mosi_{dataset_name}.pth"
        #如果test是为了获得lvu的特征，说明就是得到情绪特征，那么就不可能用在lvu上训练的模型，必须是在情绪数据集mosi（mosi_lvu)上训练的模型
    ...
    if 'lvu' in args['dataset_name']:
        dataloader = LVUDataLoader(args, num_workers)
    else:
    ...
    if get_feats == False:
        ...
        for idx, id in enumerate(results['Ids']):
            result_dict[str(int(id))] = results["SResults"][idx].tolist()
        json.dump(result_dict, open('MMSA_classification_results/'+model_name+'_'+dataset_name+'_result.json','w'))
    else:
        for mode in ['train', 'val', 'test']:
            results = trainer.do_test(model, dataloader[mode], mode="TEST", return_sample_results=True)
            out_file_dir = res_save_dir + '/' + mode + '/' + model_name+ '_multi_modal_senti_feats'
            if not os.path.exists(out_file_dir):
                os.makedirs(out_file_dir)
            for idx, id in enumerate(results['Ids']):
                result_to_save = {}
                result_to_save['vision'] = results['Features']['Feature_v'][idx]
                result_to_save['audio'] = results['Features']['Feature_a'][idx]
                result_to_save['text'] = results['Features']['Feature_t'][idx]
                out_file_path = out_file_dir + '/' + id+'.pkl'
...
def MMSA_run(
    ...
    tune_times=50, feature_T="", feature_A="", feature_V="",
    model_save_dir="", res_save_dir="", log_dir="",
    gpu_ids=[0], num_workers=4, verbose_level=1,train_mode='regression'
):
    """
    ...
    if is_tune: # run tune
        ...
        initial_args['train_mode'] = train_mode
        ...
        args = get_config_regression(model_name, dataset_name, config_file)
        args['model_save_path'] = Path(model_save_dir) / f"{args['model_name']}-{args['dataset_name']}.pth"
        #最后在checkpoints文件夹里的什么outputdim300_bertfinetune的后缀应该是我自己手动加的，代码上它们其实都保存为selfmm—mosi_lvu.pth
        args['device'] = assign_gpu(gpu_ids)
        # args['train_mode'] = 'regression' # backward compatibility. TODO: remove all train_mode in code
        args['train_mode'] = train_mode
        args['feature_T'] = feature_T
        ...
        # save result to csv
        csv_file = res_save_dir / f"{dataset_name+'_'+model_name+'_'+train_mode}.csv"
        if csv_file.is_file():
            df = pd.read_csv(csv_file)
        else:
...
def _run(args, num_workers=4, is_tune=False, from_sena=False):
    ...
    if from_sena:
        ...
        final_results['val'] = trainer.do_test(model, dataloader['val'], mode="val", return_sample_results=True)
        final_results['test'] = trainer.do_test(model, dataloader['test'], mode="TEST", return_sample_results=True)
    elif is_tune:
        # use val set to tune hyper parameters
        # results = trainer.do_test(model, dataloader['val'], mode="val")
...
#                 else:
#                     args['feature_dims'][0] = data_T['val']['text'].shape[2]
#             if args['feature_A'] != "":
#                 with open(args['feature_A'], 'rb') as f:
#                     data_A = pickle.load(f)
#                 args['feature_dims'][1] = data_A['val']['audio'].shape[2]
#             if args['feature_V'] != "":
#                 with open(args['feature_V'], 'rb') as f:
#                     data_V = pickle.load(f)
#                 args['feature_dims'][2] = data_V['val']['vision'].shape[2]
#             args['device'] = assign_gpu(gpu_ids)
...
#                     }
#                     all_features = {}
#                     for select_modes in [['train', 'val', 'test'], ['train', 'val'], ['train', 'test'], \
...
#
#
#     def DEMO_run(db_url, feature_file, model_name, dataset_name, result_id, seed, train_mode='regression'):
#
#         db_params = db_url.split('//')[1].split('@')[0].split(':')
#         db_user = db_params[0]
#         db_pass = db_params[1]
