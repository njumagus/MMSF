...
class MMIM():
        ...
        with tqdm(dataloader['train']) as td:
            for i_batch, batch_data in enumerate(td):
                ...
                if self.train_mode == 'regression':
                    ...
                    y_loss = self.criterion(results['M'], labels.long().squeeze())
    ...
    def do_train(self, model, dataloader, return_epoch_results=False):
        ...
        best_val = 1e8 if min_or_max == 'min' else 0
        while True:
            epochs += 1
            if self.args.contrast:
            ...
            # valation
            val_results = self.do_test(model, dataloader['val'], mode="VAL")
            self.scheduler_main.step(val_results['Loss'])  # Decay learning rate by valation loss
            cur_val = val_results[self.args.KeyEval]
            # save best model
            isBetter = cur_val <= (best_val - 1e-6) if min_or_max == 'min' else cur_val >= (best_val + 1e-6)
            # save best model
            if isBetter:
                best_val, best_epoch = cur_val, epochs
                # save model
            ...
            with tqdm(dataloader) as td:
                for batch_data in td:
                    ...
                    if return_sample_results:
                        ...
                        sample_results.extend(outputs["M"].cpu().detach().numpy().squeeze())
                    if self.train_mode == 'regression':
                        loss = self.criterion(outputs['M'], labels)
                    else:
                        loss = self.criterion(outputs['M'], labels.long().squeeze())
                    eval_loss += loss.item()
